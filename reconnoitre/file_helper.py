   ...
   for serv in serv_dict:
       ...
       if ("ftp" in serv):
           ...
           for port in ports:
               ...
               f.write("      [=] python3 dirsearch.py -u http://%s:%s/  -w /usr/share/wordlists/dirbuster/directory-list-lowercase-2.3-medium.txt -e all --plain-text-report=%s/%s_dirsearch.txt \n" % (ip_address, port, outputdir, ip_address))
               ...
               f.write("      [=] whatweb  %s:%s -v >> %s/%s__whatweb.txt \n" % (ip_address, port, outputdir, ip_address)) 
               f.write("      [=] curl -i %s\n" % (ip_address))
